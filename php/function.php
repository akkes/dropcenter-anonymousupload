...
function jqueryDecode($str){
	$str = str_replace('[JQUERY::AND]','&',$str);
	$str = str_replace('[JQUERY::EQUAL]','=',$str);
	$str = str_replace('[JQUERY::PLUS]','+',$str);
	return $str;
}
...
function tt($key,$parameters=null,$langage=DC_LANG){
	...
	if(isset($parameters)){
		...
		$return = '';
		$i=0;
		for($o = 0;$o<count($parametersVars);$o++){
			if($o!=0){
...
/* Constitue un nom lors de la creation des dossiers pour ne aps excraser les existantes (ex :nouveaux dossier (2)) */
